	...
	-- new combination means new additional period/validity
	-- hence the combination itself is used for grouping / building periods
select
	t.*,
    coalesce(
		lead(t.load_ts) over(partition by t.hk_customer, t.hk_badge order by t.load_ts) - interval 1 milliseconds,
		t.csb_deleted_at,
